  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-5 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          <p className="text-xs text-[#64748B] font-medium uppercase tracking-widest mb-1">Finanzas</p>
          <h1 className="text-2xl font-bold text-[#0F172A]">Honorarios</h1>
        </div>

        {/* Resumen del mes */}
        <div className="grid grid-cols-2 gap-3 mb-8">
          <div className="bg-[#E8F4E8] rounded-2xl p-4">
            <p className="text-xs text-[#64748B] mb-2">Cobrado este mes</p>
            <p className="text-2xl font-bold text-[#2D6A2D]">
              ${totalCobrado.toLocaleString('es-UY')}
            </p>
            <p className="text-xs text-[#64748B] mt-1">UYU</p>
          </div>
          <div className="bg-[#FFF7ED] rounded-2xl p-4">
            <p className="text-xs text-[#64748B] mb-2">Pendiente de cobro</p>
        ...
        {/* Lista de pagos */}
        <div>
          <h2 className="text-xs font-semibold text-[#64748B] uppercase tracking-widest mb-3">Todos los cobros</h2>
          <div className="flex flex-col gap-2">
            {payments && payments.length > 0 ? payments.map((p: any) => (
              <div key={p.id} className="bg-white rounded-2xl border border-[#E2E8F0] px-4 py-4 flex items-center gap-3">
                <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${p.status === 'paid' ? 'bg-green-500' : 'bg-orange-400'}`} />
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-[#0F172A]">${p.amount.toLocaleString('es-UY')}</p>
