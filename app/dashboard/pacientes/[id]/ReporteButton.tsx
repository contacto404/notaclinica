...
export default function ReporteButton({
  ...
  async function handleGenerate() {
    ...
    if (data.reportId) {
      const reportUrl = `${window.location.origin}/api/reporte-paciente?reportId=${data.reportId}`
      if (patientPhone) {
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-lg border border-[#F0E8E0] shadow-xl my-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={instructions} onChange={e => setInstructions(e.target.value)} rows={3}
                  placeholder="Ej: Evitar alcohol, descansar 8 horas, hacer actividad física moderada..."
                  className="border border-[#F0E8E0] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#E8602C] bg-[#FBF7F4] resize-none" />
              </div>
            ...
            <div className="flex flex-col gap-2 mt-6">
              <button onClick={handleGenerate} disabled={loading}
                className="bg-[#E8602C] text-white rounded-xl py-3 text-sm font-semibold hover:bg-[#D04F1E] disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
                {loading ? 'Generando...' : patientPhone ? '💬 Generar y enviar por WhatsApp' : '📋 Generar PDF'}
              </button>
