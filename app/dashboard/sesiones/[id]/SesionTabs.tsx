...
export default function SesionTabs({ summary, transcription, report, sessionId }: Props) {
  ...
  return (
    <div>
      ...
      {tab === 'transcripcion' && (
        <div className="bg-white rounded-3xl border border-[#EDEDED] p-6">
          <h2 className="text-xs font-semibold text-[#6E6E73] uppercase tracking-widest mb-4">Conversación</h2>
          {dialogue.length > 0 ? (
            <>
              <div className="flex flex-col gap-3">
                {dialogue.map((t, i) => {
              ...
              <p className="text-xs text-[#A3A3A3] mt-4">Conversación reconstruida automáticamente con IA · revisá antes de usar.</p>
            </>
          ) : transcription ? (
            <>
              <p className="text-sm text-[#6E6E73] leading-relaxed border-l-2 border-[#0A0A0A] pl-4 whitespace-pre-line">
                {transcription.content}
              </p>
              <p className="text-xs text-[#A3A3A3] mt-4">Transcripción automática de la sesión · revisá antes de usar.</p>
            </>
          ) : (
