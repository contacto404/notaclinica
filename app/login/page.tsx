...
function LoginForm() {
  ...
  const inputClass = "border border-[#EDEDED] rounded-xl px-3 py-2.5 text-sm text-[#0A0A0A] outline-none focus:border-[#0A0A0A] bg-[#F5F5F7]"

  return (
    <div className="min-h-screen bg-[#F5F5F7] flex items-center justify-center p-4">
      <div className="w-full max-w-3xl flex rounded-3xl overflow-hidden shadow-lg border border-[#EDEDED]">

        {/* Panel izquierdo */}
        <div className="flex-1 bg-[#0A0A0A] p-10 flex-col justify-between gap-8 hidden md:flex">
          <div>
            <img src="/logo-white-v4.png" alt="NotaClínica" className="h-9 w-auto" />
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <div key={bold} className="flex items-center gap-3 text-[#F0F0F0] text-sm">
          ...
          <p className="text-[#E5E5E5] text-xs">Para médicos de todas las especialidades</p>
        </div>

        {/* Panel derecho */}
        <div className="w-full md:w-[380px] p-10 bg-white flex flex-col justify-center gap-5 flex-shrink-0">
          <div className="md:hidden">
            <img src="/logo-v4.png" alt="NotaClínica" className="h-9 w-auto dark:hidden" />
            <img src="/logo-white-v4.png" alt="NotaClínica" className="h-9 w-auto hidden dark:block" />
          </div>
          <div>
          ...
          <form onSubmit={mode === 'login' ? handleLogin : handleRegister} className="flex flex-col gap-4">
            {mode === 'register' && (
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-[#6E6E73] font-medium">Nombre completo</label>
                <input type="text" value={name} onChange={e => setName(e.target.value)}
                  placeholder="Dr. Juan García" required className={inputClass} />
              </div>
            )}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#6E6E73] font-medium">Correo electrónico</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="tu@email.com" required className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#6E6E73] font-medium">Contraseña</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="••••••••" required minLength={6} className={inputClass} />
            </div>
            {error && <p className="text-xs text-red-500">{error}</p>}
            <button type="submit" disabled={loading}
              className="bg-[#0A0A0A] text-white rounded-xl py-3 text-sm font-semibold disabled:opacity-60 hover:bg-[#262626] transition-colors shadow-sm">
